...
TileEntity.registerPrototype(BlockID.deconstructor, {
	 ...
	 energyTick: function(type, src) {
		if(this.data.energy < 6000){
			 this.data.energy = this.data.energy + Math.floor(src.get(72));
			 this.container.setScale("energyScale", 1 / 6000 * src.get(72));
		}
		tickMath = Math.random();
		if(this.data.energy > 600){
			if(this.container.getSlot("slotSource").id == ItemID.rocket_item1){
				...
				this.data.energy = this.data.energy - 500;
	     	}
		 }
	 },
...
EnergyTileRegistry.addEnergyTypeForId(BlockID.deconstructor, energyRedstoneEU);
